...
public abstract class PircBot implements ReplyConstants {
    ...
    /**
     * Grants super-operator privileges to a user on a channel.
     * Successful use of this method may require the bot to have operator
     * status itself.
     * 
     * @param channel The channel we're opping the user on.
     * @param nick The nick of the user we are opping.
     */
    public final void aop(String channel, String nick) {
        this.setMode(channel, "+a " + nick);
    }


    /**
     * Removes super-operator privileges from a user on a channel.
     * Successful use of this method may require the bot to have operator
     * status itself.
     * 
     * @param channel The channel we're deopping the user on.
     * @param nick The nick of the user we are deopping.
     */
    public final void deAop(String channel, String nick) {
        this.setMode(channel, "-a " + nick);
    }


    /**
     * Grants half-operator privileges to a user on a channel.
     * Successful use of this method may require the bot to have operator
     * status itself.
     * 
     * @param channel The channel we're opping the user on.
     * @param nick The nick of the user we are opping.
     */
    public final void hop(String channel, String nick) {
        this.setMode(channel, "+h " + nick);
    }


    /**
     * Removes half-operator privileges from a user on a channel.
     * Successful use of this method may require the bot to have operator
     * status itself.
     * 
     * @param channel The channel we're deopping the user on.
     * @param nick The nick of the user we are deopping.
     */
    public final void deHop(String channel, String nick) {
        this.setMode(channel, "-h " + nick);
    }
...
//        }
        	...
        	String channel = splitresponse[1];
        	String login = splitresponse[2];
        	String hostname = splitresponse[3];
        	String nick = splitresponse[5];
        	String modes = splitresponse[6];
        	String prefix = "";
        	Matcher m = Pattern.compile(".*?([.+%@&~]).*?").matcher(modes);
        	if(m.matches())
        		prefix = m.group(1);
        	this.addUser(channel, new User(prefix, nick, login, hostname));
        }
            ...
            if (users != null) {
                ...
                while(enumeration.hasMoreElements()) {
                    ...
                    if (userObj.getNick().equalsIgnoreCase(nick)) {
                        if (userMode == OP_ADD) {
                            if (userObj.hasVoice()) {
                                newUser = new User("@+", nick, userObj.getLogin(), userObj.getHostname());
                            }
                            else {
                                newUser = new User("@", nick, userObj.getLogin(), userObj.getHostname());
                            }
